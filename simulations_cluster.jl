...
function create_folder_new(ip::cv.ModelParameters)
    ...
    RF = string("/data/thomas-covid/proj2/results_prob_","$(replace(string(ip.β), "." => "_"))","_scen","$(ip.isoscenario)","_herd_immu_","$(ip.herd)_mask_comp_$(replace(string(ip.maskcomp), "." => "_"))") ## 
